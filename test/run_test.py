...
with torch.no_grad():
    for i, data in enumerate(dataloader_task1):
        ...
        bounding_box.append(ats_bounding_boxes)
    ...
    for i, data in enumerate(dataloader_task2):
        ...
        road_map.append(ts_road_map)
        total_ts_road_map += ts_road_map

        if opt.verbose:
            print(f'{i} - Road Map Score: {ts_road_map:.4}')

plt.plot(bounding_box,label = 'bounding_box_score')
plt.plot(road_map,label = 'road_map_score')
plt.legend(loc='upper right', fontsize=10)
plt.show()

print(f'Bounding Box Score: {total_ats_bounding_boxes / total:.4} - Road Map Score: {total_ts_road_map / total:.4}')
