...
class UnlabeledDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        if self.first_dim == 'sample':
            ...
            image = Image.open(image_path)
            image = T.Resize((198,198))(image)
            img_size = image.size[0] #assuming square image and is a multiple of 3
            loc = img_size // 3

            crop_places = [(i*loc, j*loc, (i+1)*loc, (j+1)*loc) for i in range(3) for j in range(3)]
            patches = [image.crop(c) for c in crop_places] #each element is PIL Image
            
            patch_transform = T.Compose([T.RandomCrop((50,50))] + 
                                                           self.transform.transforms[1:])
            patches = [patch_transform(patch) for patch in patches]
            random.shuffle(patches)


            return self.transform(image), patches, index,  index % NUM_IMAGE_PER_SAMPLE
